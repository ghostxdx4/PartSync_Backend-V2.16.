...
export const getRecommendations = async (req, res) => {
  ...
  console.log('📥 Received /api/recommend payload:', {
    cpuId,
    psuId,
    moboId,
    budget,
  ...
  if (
    ...
    console.warn('❌ Bad input received:', {
      cpuId, psuId, moboId, budget
    });
  ...
  try {
    ...
    const [moboRows] = await db.query('SELECT * FROM motherboard WHERE id = ?', [moboId]);
    if (!moboRows.length) {
    ...
    const [gpuRows] = await db.query('SELECT * FROM gpu WHERE price <= ?', [budget]);

    const recommendations = gpuRows
      .map(gpu => {
        const tags = [];

        if (gpu.performance_score < cpuScore * 1.5)
          tags.push('No Bottleneck');
        ...
        return {
          ...
          price: gpu.price,
          image: gpu.image || 'https://via.placeholder.com/200',
          tags
        };
      })
      .filter(Boolean)
